...
qch=imag(m);
%---------------- TX Signal -------------------------------------
 smooth1=cos(pi/2*[-1+1./4.*[0:OverSamp-1]]); %IPOINT point filtering % note

    for ii=1:length(ich)
%       ich2((ii-1)*IPOINT+1:ii*IPOINT)=(-1)^(ii)*smooth1.*ich(ii);
%       qch2((ii-1)*IPOINT+1:ii*IPOINT)=(-1)^(ii)*smooth1.*qch(ii);
       ich2((ii-1)*OverSamp+1:ii*OverSamp)=(-1)^(ii-1)*smooth1.*ich(ii); % note
       qch2((ii-1)*OverSamp+1:ii*OverSamp)=(-1)^(ii-1)*smooth1.*qch(ii); % note
    end
% sup=zeros(1,N*OverSamp);                % upsample the data by placing...
% sup(1:OverSamp:N*OverSamp)=m;                  % ... M-1 zeros between each data point
ich21=[ich2 zeros(1,OverSamp/2)];  % note Delay 1Tb
qch21=[zeros(1,OverSamp/2) qch2];  % note Delay 1Tb
sup=ich21+j*qch21;
r=conv(chan,sup);                     % ... to get received signal
...
xs=xs*sqrt(2);
[demodata]=qpskdemod(real(xs),imag(xs),1,10000,2);
noe=sum(abs(current_data(2000:8000)-demodata(2000:8000)));
figure(1)
